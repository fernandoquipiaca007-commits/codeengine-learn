...
interface EbookReaderEdgeProps {
  productId: string;
  onBack: () => void;
}
...
export function EbookReaderEdge({ productId, onBack }: EbookReaderEdgeProps) {
  ...
  // --- SOLUÇÃO ANTI-LOOP ---
  // Substituído o ResizeObserver por um listener de window.resize.
  // O ResizeObserver estava a reagir a mudanças de scrollbar durante o zoom, criando um ciclo infinito mortal.
  useEffect(() => {
    const handleResize = () => {
  ...
  useEffect(() => {
    ...
    // Avoid running immediately on mount to let dimensions settle
  ...
  const handleDocumentLoadError = useCallback(() => {
    setTimeout(() => { setError(null); load(); }, 2000);
  }, [load]);
  ...
  return (
    <div className="fixed inset-0 flex flex-col z-[100] font-sans" style={{ backgroundColor: bgColor }}>
      <style>{`
        ...
        /* Esconder barras de scroll internas da biblioteca que possam causar reflows */
        .react-pdf__Document {
          display: flex;
