...
export function ReferralShareCard({ productId, compact = false }: ReferralShareCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`glass-panel rounded-2xl ${compact ? 'p-3' : 'p-4 sm:p-5'} border border-primary/10`}
    >
      {/* Header */}
      {!compact && (
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <div>
            <p className="font-display text-xs font-semibold text-on-surface">{t('rewardsPanel.yourReferralLink')}</p>
      ...
      {/* Link Display */}
      <div className="flex items-center gap-1.5 bg-surface-highest/50 rounded-xl p-1.5 mb-2.5">
        <input
          type="text"
          value={shareUrl}
          readOnly
          className="flex-1 bg-transparent text-[11px] text-on-surface-variant font-mono truncate outline-none"
        />
        <button
          onClick={handleCopy}
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="flex-1 py-1.5 rounded-lg bg-green-600/20 text-green-400 hover:bg-green-600/30 transition-colors flex items-center justify-center gap-1"
        >
          <MessageCircle className="w-3 h-3" />
          <span className="font-display text-[10px] font-semibold">WhatsApp</span>
        </button>
        <button
          onClick={() => handleShare('twitter')}
          className="flex-1 py-1.5 rounded-lg bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 transition-colors flex items-center justify-center gap-1"
        >
          <span className="font-display text-[10px] font-semibold">𝕏</span>
        </button>
        <button
          onClick={() => handleShare('telegram')}
          className="flex-1 py-1.5 rounded-lg bg-sky-500/20 text-sky-400 hover:bg-sky-500/30 transition-colors flex items-center justify-center gap-1"
        >
          <span className="font-display text-[10px] font-semibold">TG</span>
        </button>
        <button
          onClick={() => handleShare('email')}
          className="flex-1 py-1.5 rounded-lg bg-purple-500/20 text-purple-400 hover:bg-purple-500/30 transition-colors flex items-center justify-center"
        >
