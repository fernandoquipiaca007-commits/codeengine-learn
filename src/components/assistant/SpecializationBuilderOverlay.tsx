...
import { 
  ...
  Trash2, Plus, Edit2, ShoppingCart, X, HelpCircle, BookOpen
} from 'lucide-react';
...
export function SpecializationBuilderOverlay({
  ...
  const [mode, setMode] = useState<'building' | 'done' | 'edit'>('building');
  ...
  // 1. Fetch catalog and start build logs
  useEffect(() => {
    if (!isOpen) return;
    ...
    const logInterval = setInterval(() => {
      if (currentLogIndex < assemblyLogs.length) {
        setLogs(prev => [...prev, assemblyLogs[currentLogIndex]]);
        setCurrentLogIndex(curr => curr + 1);
      } else {
  ...
  async function generateSyllabus() {
    try {
      ...
      const response = await fetch(`${backendUrl}/api/assistant/generate-syllabus`, {
        ...
        body: JSON.stringify({ objective, level, time }),
      });
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-display text-sm sm:text-base font-bold text-white uppercase tracking-wider">
                  {mode === 'building' ? 'Montando Especialização...' : 'Sua Especialização sob Medida'}
                </h3>
            ...
            {mode === 'building' && (
              <div className="space-y-6 py-8">
                ...
                <div className="rounded-xl border border-white/5 bg-black/30 p-4 font-mono text-[10px] sm:text-xs text-green-400 space-y-2 h-[160px] overflow-y-auto scrollbar-none shadow-inner">
                  ...
                  {currentLogIndex < assemblyLogs.length && (
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className="font-sans text-[10px] text-green-400 font-semibold bg-green-500/10 border border-green-500/20 px-2 py-0.5 rounded-full">
                        Economize 40%
                      </span>
                    </div>
